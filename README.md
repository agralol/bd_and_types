# Домашнее задание к занятию "`Базы данных, их типы`" - `Chernov Vyacheslav`
...
1. Пользователь инициирует запрос на пополнение счёта телефона на сумму Х рублей.
2. Оператор сотовой связи отправляет запрос в банк/платёжную систему на списание суммы Х со счёта пользователя.
3. Банк проверяет наличие средств на счёте пользователя и списывает сумму Х.
4. Банк отправляет подтверждение успешного списания оператору сотовой связи.
5. Оператор сотовой связи зачисляет сумму Х на счёт телефона пользователя.
6. Оператор отправляет пользователю подтверждение успешного пополнения баланса.
...
1. Система оператора сотовой связи проверяет условия для автоплатежа (текущий баланс, время, минимальная сумма).
2. Оператор отправляет запрос в банк на списание заранее согласованной суммы со счета пользователя.
3. Банк проверяет наличие средств на счете пользователя и выполняет списание суммы.
4. Банк отправляет подтверждение оператору сотовой связи о успешном списании.
5. Оператор зачисляет полученную сумму на счет телефона пользователя.
6. Оператор отправляет уведомление пользователю об успешном автоплатеже.
